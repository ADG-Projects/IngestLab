...
2. **Matches JSON** (`outputs/unstructured/<doc>.matches.json`)
   ...
   - `run_config`: metadata about how the run was produced
     ...
     - Language hints mirrored from the UI: `primary_language` (`eng` or `ara`), `ocr_languages` (string passed to Tesseract, e.g., `ara+eng`), `languages` (comma list or array of ISO codes forwarded to Unstructured), and `detect_language_per_element` (bool). Azure runs also capture `detected_languages` and `detected_primary_language` from the service output so previews can auto-switch to RTL when detection reports Arabic.
