...
from ..config import DATASET_DIR, DEFAULT_PROVIDER, get_out_dir
...
def api_matches(slug: str, provider: str = Query(default=None)) -> Dict[str, Any]:
...
def api_tables(slug: str, provider: str = Query(default=None)) -> List[Dict[str, Any]]:
    path = resolve_slug_file(slug, "{slug}.pages*.tables.jsonl", provider=provider or DEFAULT_PROVIDER)
...
def pdf_for_slug(slug: str, provider: str = Query(default=None)):
    pdf_path = resolve_slug_file(slug, "{slug}.pages*.pdf", provider=provider or DEFAULT_PROVIDER)
